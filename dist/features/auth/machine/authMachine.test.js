...
describe("Auth Machine", () => {
    ...
    describe("Login Flow", () => {
        ...
        it("should return to idle with an error on failure", async () => {
            ...
            const actor = createTestActor();
            await waitForState(actor, (s) => s.matches("unauthorized"));
            const p1 = waitForState(actor, (s) => s.matches({ unauthorized: { login: "idle" } }) && s.context.error !== null);
            actor.send({ type: "LOGIN", payload: { email: "a", password: "b" } });
            await p1;
    ...
    describe("Forgot Password Flow", () => {
        ...
        it("should ignore RESET_PASSWORD when no action token exists", async () => {
            ...
            await Promise.resolve();
            expect(actor.getSnapshot().matches({ unauthorized: { forgotPassword: "resetPassword" } })).toBe(true);
...
describe("resolveRegistrationPassword", () => {
    it("returns provided password when non-empty string", () => {
        expect((0, authMachine_1.resolveRegistrationPassword)({ email: "a", password: "Secret123" })).toBe("Secret123");
    });
    it("returns empty string when password is missing or blank", () => {
        expect((0, authMachine_1.resolveRegistrationPassword)({ email: "a", password: "" })).toBe("");
        expect((0, authMachine_1.resolveRegistrationPassword)({ email: "a" })).toBe("");
        expect((0, authMachine_1.resolveRegistrationPassword)(undefined)).toBe("");
    });
});
