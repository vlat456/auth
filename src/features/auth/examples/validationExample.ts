/**
 * Example implementation of how to use safety utilities in auth machine
...
export const createExampleAuthMachine = (authRepository: IAuthRepository) => {
  return setup({
    types: {
    ...
    actions: {
      setSession: assign({
        session: ({ event }) => {
          ...
          const output = safeExtractOutput<AuthSession>(event as any);
