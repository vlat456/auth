...
import {
  ...
  validateRefreshResponseData,
  formatValidationErrors,
  getFirstValidationError,
  validateBatch,
} from "./zodHelpers";
import { LoginRequestSchema } from "../schemas/validationSchemas";
...
describe("Zod Helpers", () => {
  ...
  describe("validateRequestOtp", () => {
    it("should validate correct OTP request", () => {
      const result = validateRequestOtp({ email: "test@example.com" });
      expect(result.success).toBe(true);
    });

    it("should fail with invalid email", () => {
      const result = validateRequestOtp({ email: "invalid" });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe("validateVerifyOtp", () => {
    it("should validate correct verify OTP request", () => {
      const result = validateVerifyOtp({
        email: "test@example.com",
        otp: "123456",
      });
      expect(result.success).toBe(true);
  ...
  describe("validateCompletePasswordReset", () => {
    it("should validate correct password reset completion", () => {
      const result = validateCompletePasswordReset({
        actionToken: "verylongactiontoken123456",
        newPassword: "newpassword123",
      });
      expect(result.success).toBe(true);
  ...
  describe("validateLoginResponse", () => {
    it("should validate correct login response", () => {
      const result = validateLoginResponse({
  ...
  describe("validateRefreshResponseData", () => {
    it("should validate correct refresh response", () => {
      const result = validateRefreshResponseData({
  ...
  describe("getFirstValidationError", () => {
    it("should return first error from failed validation", () => {
      const result = validateLoginRequest({ email: "invalid" });
      if (!result.success) {
    ...
    it("should return first available error message", () => {
      const result = validateLoginRequest({ email: "invalid", password: "a" });
      if (!result.success) {
        const error = getFirstValidationError(result);
        expect(error).toBeDefined();
  ...
  describe("validateBatch", () => {
    it("should validate array of valid items", () => {
      ...
      const result = validateBatch(LoginRequestSchema, items);
