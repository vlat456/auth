...
class MockAuthRepository implements IAuthRepository {
  ...
  async requestPasswordReset(payload: { email: string }): Promise<void> {
    ...
    if (this.errors.some(e => e.method === 'requestPasswordReset')) {
      const error = this.errors.find(e => e.method === 'requestPasswordReset');
  ...
  async verifyOtp(payload: { email: string; otp: string }): Promise<string> {
    ...
    const actionToken = `action_${payload.email.replace('@', '_')}_${Date.now()}`;
    this.tokens[actionToken] = true;
    return actionToken;
  }

  async completeRegistration(payload: { actionToken: string; newPassword: string }): Promise<void> {
    this.calls.push(`completeRegistration:${payload.actionToken}`);
    
    if (this.errors.some(e => e.method === 'completeRegistration')) {
      const error = this.errors.find(e => e.method === 'completeRegistration');
  ...
  async completePasswordReset(payload: { actionToken: string; newPassword: string }): Promise<void> {
    this.calls.push(`completePasswordReset:${payload.actionToken}`);
    
    if (this.errors.some(e => e.method === 'completePasswordReset')) {
      const error = this.errors.find(e => e.method === 'completePasswordReset');
...
describe("Auth Machine Integration Tests", () => {
  ...
  describe("Checking Session States", () => {
    ...
    it("should transition to unauthorized when checkSession fails", async () => {
      mockRepo.errors.push({ method: "checkSession", error: new Error("Network error") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
  ...
  describe("Login Flow States", () => {
    ...
    it("should return to idle with error on login failure", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "login", error: new Error("Invalid credentials") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
  ...
  describe("Registration Flow States", () => {
    it("should handle complete registration flow with OTP verification", async () => {
      ...
      expect(mockRepo.calls.some(call => call.startsWith("completeRegistration:"))).toBe(true);
    ...
    it("should handle registration failure in form state", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "register", error: new Error("Email already exists") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
    ...
    it("should handle OTP verification failure", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "verifyOtp", error: new Error("Invalid OTP code") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
      ...
      expect(actor.getSnapshot().context.error?.message).toBe("Invalid OTP code");
    });

    it("should handle registration completion failure", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "completeRegistration", error: new Error("Invalid token") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
  ...
  describe("Forgot Password Flow States", () => {
    it("should handle complete forgot password flow", async () => {
      ...
      expect(mockRepo.calls.some(call => call.startsWith("completePasswordReset:"))).toBe(true);
      expect(mockRepo.calls).toContain("login:user@test.com");
      expect(actor.getSnapshot().value).toEqual("authorized");
    });
    ...
    it("should handle forgot password request failure", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "requestPasswordReset", error: new Error("Email not found") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
      ...
      expect(mockRepo.calls).toContain("requestPasswordReset:nonexistent@test.com");
    ...
    it("should handle OTP verification failure in forgot password flow", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "verifyOtp", error: new Error("Invalid OTP") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
    ...
    it("should handle password reset completion failure", async () => {
      (mockRepo as any).session = null;
      mockRepo.errors.push({ method: "completePasswordReset", error: new Error("Token expired") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
  ...
  describe("Authorized State and Logout", () => {
    ...
    it("should handle logout failure", async () => {
      (mockRepo as any).session = { accessToken: "valid-token" };
      mockRepo.errors.push({ method: "logout", error: new Error("Network error during logout") });
      const actor = createActor(createAuthMachine(mockRepo));
      actor.start();
      ...
      expect(mockRepo.calls).toContain("logout");
      expect(actor.getSnapshot().value).toEqual("authorized");
      expect(actor.getSnapshot().context.error?.message).toBe("Network error during logout");
    });
  });
  ...
  describe("Navigation Between States", () => {
    it("should navigate between login, register, and forgot password", async () => {
      ...
      await waitForState(actor, (s) => s.matches({ unauthorized: { login: "idle" } }));
      expect(actor.getSnapshot().value).toEqual({
        unauthorized: { login: "idle" }
      });
      ...
      // Go back to login
      const p2 = waitForState(actor, (s) => s.matches({ unauthorized: { login: "idle" } }));
      ...
      const p3 = waitForState(actor, (s) => s.matches({ unauthorized: { forgotPassword: "idle" } }));
      actor.send({ type: "GO_TO_FORGOT_PASSWORD" });
      await p3;
      expect(actor.getSnapshot().value).toEqual({
      ...
      const p4 = waitForState(actor, (s) => s.matches({ unauthorized: { login: "idle" } }));
      actor.send({ type: "GO_TO_LOGIN" });
      await p4;
      expect(actor.getSnapshot().value).toEqual({
        unauthorized: { login: "idle" }
    ...
    it("should clear context when navigating between states", async () => {
      ...
      expect(actor.getSnapshot().matches({ unauthorized: { login: "idle" } })).toBe(true);
    });
  });
  ...
  describe("Error Handling States", () => {
    it("should handle errors in verifyOtp during registration with guards", async () => {
      ...
      mockRepo.errors.push({ method: "verifyOtp", error: new Error("OTP expired") });
  ...
  describe("Edge Cases and Error Recovery", () => {
    ...
    it("should handle network errors during session check", async () => {
      mockRepo.errors.push({ method: "checkSession", error: new Error("Network timeout") });
    ...
    it("should handle empty strings for credentials", async () => {
      ...
      expect(mockRepo.calls.some(call => call.startsWith("login:"))).toBe(true);
    }, 15000); // Increase timeout to 15 seconds
  });
  ...
  describe("Token Expiry and Refresh Scenarios", () => {
    ...
    it("should handle token expiry during forgot password flow", async () => {
      ...
      expect(actor.getSnapshot().matches({ unauthorized: { forgotPassword: "verifyOtp" } })).toBe(true);
    });
  });
  ...
  describe("Concurrent and Rapid State Transitions", () => {
    it("should handle rapid events without race conditions", async () => {
      ...
      expect(actor.getSnapshot().matches({ unauthorized: { forgotPassword: "idle" } })).toBe(true);
    ...
    it("should handle multiple login attempts concurrently", async () => {
      ...
      expect(mockRepo.calls.filter(call => call.startsWith("login:")).length).toBeGreaterThanOrEqual(1);
    });
  });
  ...
  describe("Advanced Error Scenarios", () => {
    it("should handle repository method failures during refresh token flow", async () => {
      ...
      mockRepo.errors.push({ method: "refresh", error: new Error("Refresh token expired") });

      // Try to call refresh directly to test the error handling
      await expect(mockRepo.refresh("refresh_123")).rejects.toThrow("Refresh token expired");
    ...
    it("should handle profile refresh failures gracefully", async () => {
      ...
      mockRepo.errors.push({ method: "refreshProfile", error: new Error("Profile server unavailable") });

      // Trigger a manual profile refresh to test the scenario - should throw
      await expect(mockRepo.refreshProfile()).rejects.toThrow("Profile server unavailable");
